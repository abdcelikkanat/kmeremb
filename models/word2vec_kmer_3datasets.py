...
if __name__ == "__main__":
    ...
    filename = f"3genomes_samples={dataset_sample_size}_intkmerseq_k={k}" #f"zymohmw_samples={dataset_sample_size}_intkmerseq_k={k}"  # f"3genomes_samples=1000_intkmerseq_k={k}"
    ...
    # Save the embeddings
    with open(
            f"./embs/5___{filename}_{'' if normalize else 'un'}normalized_w={window_size}_"
    ...
    for color_idx, color in enumerate(['red', 'orange', 'green', 'blue', 'purple', 'brown', 'pink', 'gray'][:class_num]):
        plt.scatter(
    ...
    # plt.show()
    plt.savefig(
        f"./figures/5___{filename}_{'' if normalize else 'un'}normalized_w={window_size}_"
