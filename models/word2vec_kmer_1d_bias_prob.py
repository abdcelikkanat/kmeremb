...
class KMerEmb1D(torch.nn.Module):
    def __init__(self,  kmer_num, dim=2,
        ...
        self.__bias = torch.nn.Parameter(
            2 * torch.rand(size=(self.__kmer_num, ), device=self.__device) - 1, requires_grad=True
        )
    ...
    def __compute_loss(self, x, degrees):
        ...
        bias_sum = self.__bias[x[:, 0]] + self.__bias[x[:, 1]]

        rate = torch.exp(bias_sum - dist)
...
def compute_pair_counts(window_size, k, current_seq):
    ...
    pair_counts = torch.zeros(size=(4 ** k, 4 ** k), dtype=float)
...
if __name__ == "__main__":
    ...
    init_time = time.time()
    pair_counts = np.zeros(shape=(4 ** k, 4 ** k), dtype=float)
    for current_seq, current_prob in zip(sequences, probs):
        for i in range(len(current_seq)):
            # Get the center
            center = current_seq[i]
            for j in range(max(0, i - window_size), min(len(current_seq), i + window_size + 1)):
                ...
                if center <= context:
                    pair_counts[center, context] += current_prob[i] * current_prob[j]
                    # print(current_prob[i], current_prob[j])
                    # raise ValueError
                    # if current_prob[i] < .9:
                    #     print(current_prob[i])
    ...
    # Get the embeddings
    embs = kmer_model.get_emb()

    print(embs)
    print(embs.shape)

    # Define the output file name
    output_filename = f"prob_bias_{filename}_{'' if normalize else 'un'}normalized_w={window_size}_lr={lr}_epoch={epoch_num}_batch={batch_size}"
